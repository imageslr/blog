...
这样一来，我们就明确了关键结果。接下来，我们就可以针对 *每个* 关键结果，进一步分解出可执行的行动了。分解行动时，也可以按照自上而下的模式，先将其分解成不同的子任务或不同的阶段，然后再针对每个子任务 / 阶段，拆分出具体的 TODO。
...
{: .mb-0}
* OKR 是一种自上而下分解的方法，是一种以终为始的反向思维。这与我们习惯的自下而上整合的模式不同。
