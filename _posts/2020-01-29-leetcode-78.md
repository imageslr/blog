...
### 解法二：递归（回溯算法）
对于每个元素，有「选」或者「不选」两种情况。因此可以用回溯法来做。
* 每选择一个元素，会得到一个新的组合，将其加入结果集
* 在每一层，遍历 `[start,n)` 的每个元素 `i`，选择它，然后进入下一层
* 下一层只能选择 `[i+1,n)` 之间的元素
* 从下一层返回时，删除刚才选的这个元素（不选择它，回溯），继续选择下一个元素
...
func dfs(nums []int, start int) {
	tmp := copy(stack)
	res = append(res, tmp) // 每层有一个结果
	for i := start; i < len(nums); i++ { // 从 start 开始遍历
		stack = append(stack, nums[i]) // 选择
		dfs(nums, i+1) // 只允许在之后的元素里遍历
		stack = stack[:len(stack)-1] // 回溯，取消选择
	}
}
```
