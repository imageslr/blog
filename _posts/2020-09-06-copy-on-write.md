...
## 为什么需要 Copy-on-write？
物理内存资源对于计算机来说是十分宝贵的。当通过 `fork()` 来创建一个子进程时，操作系统需要将父进程虚拟内存空间中的大部分内容全部拷贝到子进程中（主要是数据段、堆、栈；代码段共享）。这个操作不仅非常耗时，而且会浪费大量物理内存——因为这些物理内存的内容是完全相同的，完全可以在不同进程间共享这些物理内存。

但是实际上，不同进程的内存空间应该是独立/**私有**的。对于上面这些共享的内存区域，多个进程读取没有任何问题，但是如果有进程尝试写入，那么可能会破坏其他进程的内存空间。

因此引入了写时复制技术。只要没有进程修改这部分私有区域，就可以继续共享物理内存中的同一个副本；然而只要有一个进程时图写私有区域内的某个页面，那么就会为这个进程创建该页面的一个新副本。
