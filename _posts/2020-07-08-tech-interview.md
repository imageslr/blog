...
<img src="/media/16050117063362.jpg" alt="-w406" style="width:400px;" />
...
**索引的分类**：[MySQL 索引分类，90% 的开发都不知道](https://zhuanlan.zhihu.com/p/115746492)
...
* 匹配最左前缀（联合索引）
* 仅针对索引列进行查询
* 其他：[面试官出的 MySQL 索引问题 - 索引的适用场景](https://juejin.im/post/5da5b61e518825798038f072#heading-24)

**索引的适用场景：**

适合建立索引：
1. 经常作为查询条件的字段。在不同值较多的列上建立索引，不同值少的列上不要建立索引。比如在雇员表的“性别”列上只有“男”与“女”两个不同值，就无必要建立索引。如果建立索引不但不会提高查询效率，反而会严重降低**更新**速度。
2. 频繁进行排序或分组（即进行 group by 或 order by 操作）的列
3. 如果待排序的列有多个，可以在这些列上建立联合索引 (compound index)

不适合建立索引：
1. 更新非常频繁的字段
5. 不会作为查询条件的字段 (不会出现在 WHERE 子句中)
6. 表记录很少的时候，没必要添加索引
...
如果频繁地使用相同的几个字段查询，就可以考虑建立这几个字段的联合索引来提高查询效率。比如对于联合索引 `test_col1_col2_col3`，实际建立了 `(col1)`、`(col1, col2)`、`(col, col2, col3)` 三个索引。

**最左匹配原则**是指：这些索引能够被包含 `col1`、`(col1 col2)`、`(col1 col2 col3)` 的查询利用到，但是不能够被 `col2`、`(col2、col3)` 的等值查询利用到。这与底层实现有关。

联合索引实际上相当于将**多个索引列视作一个列**。叶节点中的每个行记录，也是按照这些索引列组合后的**字典序**进行比较的。字典序本身就符合最左匹配原则。

联合索引底层还是一棵 B+ 树。联合索引将多个索引列视为一个，所以非叶节点存储的是多个索引列 `(col1、col2、col3)` 的元组，叶节点存储的是索引列 `col1`、`col2`、`col3` 的值以及主键 id。叶节点之间按照 `col1`、`col2`、`col3` 的字典序排序。下图是一个简单的例子：

![](/media/07-13-23-54-58.png)

**联合索引的主要优势**是减少结果集数量：如果根据 `col1`、`col2`、`col3` 的单列索引进行查询，需要分别得到 `num[i]` 个结果，然后再取交集；而如果使用联合索引查询，只会得到很少的一段数据。见下图：

![](/media/07-14-00-00-03.png)

联合索引属于：
* 覆盖索引：如果 `select a, b, c from table` 且 `(a b c)` 是联合索引的*前缀列* (比如联合索引是 `(a, b, c, d)`)，那么不用进行回表查询，直接可以从索引表中获得这几个列的值
* 辅助索引：叶节点存储主键的值

**数据库查询 offset 的流程**：[MySQL 查询时，offset 过大影响性能的原因与优化方法](https://www.huaweicloud.com/articles/a0e380c6a745cc828aa2031176de421f.html)
1. 查询 offset 的流程：
   1. 先通过辅助索引找到*所有*主键值
