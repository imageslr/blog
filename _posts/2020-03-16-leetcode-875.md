...
对于本题而言，`left`、`right` 表示 `吃香蕉的速度`，由于每小时最少吃一根香蕉，最多只能吃一堆香蕉，所以 `left`、`right` 的初值分别为 `1`、`MaxOf(piles)`。

重点在于 `if` 的判断条件。假设已知在 `H` 小时内「恰好」吃掉所有香蕉的速度为 `targetSpeed`，那么判断条件可以写为：`if mid >= targetSpeed`，表示查找「第一个大于等于 `targetSpeed` 的整数速度 `K`」。由于**「速度」和「时间」成反比，所以 `if mid >= targetSpeed` 等同于 `if TotalTime(piles, mid) <= H`**。

可视化的解释：

![-w435](/media/15843778032171.jpg)

上图为吃香蕉的速度与时间的关系图，吃香蕉的最长时间 `H` 与其对应的速度 `targetSpeed` 用两个蓝点表示。
