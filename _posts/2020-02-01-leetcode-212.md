...
解法二的搜索过程中，前缀树的节点可能被重复访问，需要使用一个 `HashMap` 去重。可以在每次找到某个单词后，令 `node.isLast = false`，这样不需要额外的 `HashMap` 也能去重。
```diff
if node.isLast { // 如果子节点表示一个单词，则将其加入到结果中
-    resMap[nextPrefix] = 1
...
此外，搜索过程中需要维护一个字符串 `prefix`，但字符串的唯一作用是在找到 `node.isLast == true` 的节点时，获取该节点所表示的单词。其实可以直接将完整单词保存在前缀树的 `node` 里，就不需要 `prefix` 了。在字典里包含特别长的单词的时候，避免频繁地构建字符串可能提升运行速度。
