...
定义两个变量 `left`、`right` 分别表示上图两个虚线，始终令 `left` 右侧递减，`right` 左侧递增，只遍历区间 `[left,right]`。这是一种“双指针”的思路。当 `left >= right` 时，说明所有柱子中已经不存在“谷”了，可以结束了。
...
对于每一个柱子，只需要向左、向右找到最高的柱子，如果左右最高的柱子都比它高，那么这个柱子上方一定可以接到雨水。能接多少雨水，取决于左右最高的柱子的**较小值**。设柱子 `i` 左右最高的柱子下标分别为 `left`、`right`，则柱子 `i` 可以接雨水：
```
...
**注意**：第 3 步弹出栈顶元素后，必须判断栈是否为空。只有栈不为空，才能保证弹出元素左侧有比它更高的元素，这时才能接到雨水。
