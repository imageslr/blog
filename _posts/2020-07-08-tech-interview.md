...
允许系统中同时存在四个必要条件，但是每当进程提出资源申请时,系统要分析满足该资源请求系统是否会发生死锁，若不会发生则实施分配,否则拒绝分配。银行家算法实现了这个过程。
...
- 外部碎片是动态分区法产生的，指被占用分区之间的小空间，虽然可以被使用，但是由于大小太小而无法被分配
