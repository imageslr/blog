...
在单线程的场景下，函数调用都是同步调用。如下所示，当 A 调用 B 后，无论 `B()` 会执行多长时间，A 都会**等待**，直到其返回执行结果。“等待”其实就是串行执行/顺序执行。
...
I/O 多路复用可以同时注册多个文件描述符，并且哪个文件描述符先就绪是不确定的，符合“非线性执行序列”，那它是不是异步的？不是，因为从程序的角度来看，I/O 多路复用不具有通知机制。进程需要主动调用 select 检查哪些描述符就绪，并且这个过程中进程会被阻塞。
...
所以 I/O 多路复用是：多路、同步、阻塞的模型。相对的，阻塞 I/O 是：单路、同步、阻塞的模型。
