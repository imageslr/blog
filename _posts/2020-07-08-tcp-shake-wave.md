...
* 数据传输阶段，序列号 = 第一个报文段的序列号 + 已经发送的字节数
    ...
    * 如果某个报文段不携带数据，不会消耗序列号的，下一个报文段还是用相同的序列号发送
...
原理：
* 在三次握手过程中，服务器发送 `[SYN/ACK]` 包（第二个包）之后、收到客户端的 `[ACK]` 包（第三个包）之前的 TCP 连接称为半连接（half-open connect），此时服务器处于 `SYN_RECV`（等待客户端相应）状态。如果接收到客户端的 `[ACK]`，则 TCP 连接成功，如果未接受到，则会**不断重发请求**直至成功
...
### 为什么需要四次挥手
为 **TCP 是全双工的**，一方关闭连接后，另一方还可以继续发送数据。所以四次挥手，将断开连接分成两个独立的过程。
...
以及：如果三次握手阶段、四次挥手阶段的包丢失了怎么办？比如上面描述的“服务端重发 FIN”的问题。
...
1. 客户端向服务端发送 HTTP 请求头，大小 77 字节，序列号为 1（`seq 1:78` 是一个左闭右开的区间）；`Flags [P.]` 表示 `PSH=1, ACK=1`，`PSH` 标志位的作用是告诉服务端收到这个数据包后立刻处理，因为已经没有进一步要发的数据包了
2. 服务端收到客户端的数据包后，立刻先返回一个 ACK 包，确认号 `ack=78`，length 为 0
3. 服务端处理完请求后，依次返回给客户端两个长度为 1360 字节的数据包，序列号分别为 `seq=1、seq=1361`
4. 客户端收到这两个数据包后，立刻返回一个 ACK 包，确认号 `ack=2721`，length 为 0。这个确认号表示已经收到了服务端发来的 2720 字节的数据（1360+1360=2720）
5. 服务端再发送一个长度为 61 字节的数据包，序列号为 `seq=2721`，服务端数据传输完毕
6. 客户端收到数据包后，立刻返回一个 ACK 包，确认号 `ack=2782`，客户端数据接收完毕
