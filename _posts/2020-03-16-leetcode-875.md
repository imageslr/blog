...
本文将通过两个实际问题，深入讲解如何运用这个二分查找模板代码。
...
显然，一种比较简单的方式是从 1 开始，遍历所有可能的速度，返回第一个可以在 `H` 小时内吃掉所有香蕉的速度。

不过观察本题，我们容易发现：速度越快，吃掉所有香蕉的时间就越短。也就是说，**搜索区间是单调递减的，可以使用二分查找**。另外，本题要找的是可以在 `H` 小时内吃掉所有香蕉的**最小速度**，这实际上就是要查找**下界**。所以可以直接套用上面的模板代码。

对于本题而言，`left`、`right` 表示 `吃香蕉的速度`，由于每小时最多只能吃一堆香蕉，所以 `left`、`right` 的初值分别为 `1`、`MaxOf(piles)`。

重点在于 `if` 的判断条件应该怎么写。假设已知在 `H` 小时内「恰好」吃掉所有香蕉的速度为 `targetSpeed`，那么判断条件可以写为：`if mid >= targetSpeed`，表示查找**第一个大于等于 `targetSpeed` 的速度 `K`**。

由于**「速度」和「时间」成反比，所以上面的判断条件可以改写为 `if TotalTime(piles, mid) <= H`，这两个判断的含义完全相同**。
...
下图为吃香蕉的速度与时间的关系图，题目规定的最长时间 `H` 与其对应的最小速度 `targetSpeed` 如图所示。

设吃香蕉的速度为 `K`，则当 `K >= targetSpeed` 时都可以在 `H` 小时内吃完所有的香蕉。而本题规定速度只能是整数，所以我们要找的是满足 `K >= targetSpeed` 的**第一个**整数速度，也相当于是满足 `TotalTime(piles, K) <= H` 的**第一个**整数速度。

![-w435](/media/15843778032171.jpg)
...
本题和 875 题很像，要返回能在 `D` 天内将传送带上的所有包裹送达的船的**最低运载能力**，这就相当于是在找下界，可以套用模板代码：
...
## 总结
本文通过两个实际问题，深入讲解了如何运用二分查找模板代码。我们发现，**模板代码可以优化为如下形式，从而适用于任何「查找下界」的问题**：
```go
func LowerBound(nums []int, target int) int {
    left, right := MIN, MAX // 返回值可能取的最小值、最大值
    for left <= right {
        mid := left + (right-left) >> 1
...
上面的代码中有三处变量 `MIN`、`MAX`、`IS_OK`，根据题目要求填写：
* [LeetCode 34 题](https://leetcode-cn.com/problems/find-first-and-last-position-of-element-in-sorted-array/)：查找满足 `x >=target` 的第一个位置，返回值是下标，故 `MIN`、`MAX`、`IS_OK` 分别为 `0`、`n-1`、`nums[mid] >= target`
    * 如果要查找满足 `x >=target` 的第一个位置，`IS_OK` 变为 `nums[mid] > target`
* [LeetCode 875 题](https://leetcode-cn.com/problems/koko-eating-bananas/)：查找可以在 `H` 小时内吃掉所有香蕉的最小速度，返回值是速度，故 `MIN`、`MAX`、`IS_OK` 分别为 `1`、`MaxOf(piles)`、`TotalTime(piles, mid) <= H`
* [LeetCode 1011 题](https://leetcode-cn.com/problems/capacity-to-ship-packages-within-d-days/)：查找可以在 `D`  天内运损完所有货物的最小运载能力，返回值是载重量，故 `MIN`、`MAX`、`IS_OK` 分别为 `MaxOf(weights)`、`SumOf(weights)`、`TotalTime(weights, mid) <= D`

**当题目要查找「最小值」、「第一个」时，就说明要查找「下界」，此时就可以使用上面的模板代码。**
