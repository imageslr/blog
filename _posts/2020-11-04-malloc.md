...
`brk` 用于返回堆的顶部地址；`sbrk` 用于扩展堆，通过参数 `increment` 指定要增加的大小，返回扩展前的堆顶地址。
...
![-w337](/media/16046511227960.jpg){: width="600"}
...
另一种实现方式是将内存分块。每个块的大小都是 word size（如 8 字节）的倍数，相同大小的内存块维护在同一个链表中，全局有多个内存块链表。分配时，以块为单位。这样最多只会浪费 50% 的空间，但会提高分配效率，同时减少外部碎片，避免内存合并的开销。
...
`malloc` 分配的函数应该是字对齐的。最简单的方式是先让堆的起始位置字对齐，然后始终分配 word size 的倍数的内存。
